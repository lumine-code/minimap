describe("minimap", () => {
  ...
  describe("stand-alone minimaps", () => {
    it("creates an independent minimap with its own screen size", () => {
      const standAlone = mainModule.standAloneMinimapForEditor(editor);

      expect(standAlone).not.toBe(minimap);
      expect(standAlone.isStandAlone()).toBe(true);

      const view = atom.views.getView(standAlone);
      expect(view.hasAttribute("stand-alone")).toBe(true);

      standAlone.setScreenHeightAndWidth(100, 50);
      expect(standAlone.getScreenHeight()).toBe(100);
      expect(standAlone.getScreenWidth()).toBe(50);

      expect(standAlone.getHeight()).toBe(editor.getScreenLineCount() * minimap.getLineHeight());
      expect(standAlone.getMaxScrollTop()).toBe(standAlone.getHeight() - 100);

      standAlone.setScrollTop(42);
      expect(standAlone.getScrollTop()).toBe(42);
  ...
  describe("provided service", () => {
    it("exposes the minimap API", () => {
      const service = mainModule.provideMinimap();

      expect(typeof service.observeMinimaps).toBe("function");
      expect(typeof service.registerPlugin).toBe("function");
      expect(service.minimapForEditor(editor)).toBe(minimap);
      expect(service.minimapForEditorElement(editorElement)).toBe(minimap);
      expect(service.getActiveMinimap()).toBe(minimap);
      expect(minimap instanceof service.minimapClass()).toBe(true);
    });

    it("calls observers with existing and future minimaps", async () => {
      const service = mainModule.provideMinimap();
      const observed = [];
      const disposable = service.observeMinimaps((m) => observed.push(m));

      expect(observed).toContain(minimap);
      ...
      expect(observed).toContain(mainModule.minimapForEditor(otherEditor));
  ...
  describe("plugins", () => {
    ...
    it("activates registered plugins and exposes a toggle command and setting", () => {
      const plugin = buildPlugin();
      mainModule.registerPlugin("dummy", plugin);

      expect(plugin.isActive()).toBe(true);
      expect(atom.config.get("minimap.plugins.dummy")).toBe(true);

      atom.commands.dispatch(workspaceElement, "minimap:toggle-dummy");
      expect(plugin.isActive()).toBe(false);
      expect(atom.config.get("minimap.plugins.dummy")).toBe(false);

      atom.commands.dispatch(workspaceElement, "minimap:toggle-dummy");
      expect(plugin.isActive()).toBe(true);

      mainModule.unregisterPlugin("dummy");
      expect(mainModule.plugins.dummy).toBeUndefined();
    });

    it("emits plugin lifecycle events", () => {
      const addedSpy = jasmine.createSpy("did-add-plugin");
      const removedSpy = jasmine.createSpy("did-remove-plugin");
      mainModule.onDidAddPlugin(addedSpy);
      mainModule.onDidRemovePlugin(removedSpy);

      const plugin = buildPlugin();
      mainModule.registerPlugin("dummy", plugin);
      expect(addedSpy).toHaveBeenCalledWith({ name: "dummy", plugin });

      mainModule.unregisterPlugin("dummy");
      expect(removedSpy).toHaveBeenCalledWith({ name: "dummy", plugin });
    });
  });
