...
import { StyleReader } from "./deps/style-reader";
...
export function deactivate() {
  ...
  remeasureTextEditors();
}

export function getConfigSchema() {
  return atom.packages.getLoadedPackage("minimap").metadata.configSchema;
}
...
/**
 * Forces the editor components to re-measure once the minimap elements have been added or removed.
 *
 * A component only watches its own `atom-text-editor` element for resizes, and that element keeps
 * its size when the minimap flex sibling appears or disappears. Without this nudge the component
 * keeps rendering at the width it measured while the minimap was there, until something else
 * resizes the editor. The measure is deferred to the next frame so that it also sees the package
 * stylesheet removal that follows `deactivate()`.
 *
 * @access private
 */
function remeasureTextEditors() {
...
/**
 * Returns the minimap service object.
 *
 * @returns {Object} The minimap service object.
 */
export function provideMinimap() {
