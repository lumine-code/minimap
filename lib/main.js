...
const { treeSitterHint } = require("./performance-monitor");
...
function initSubscriptions() {
  ...
  subscriptions.add(
    ...
    // variant attribute on the document root through `lumine.themes.updateAppearance` — only reports
    // itself there.
    lumine.themes.onDidChangeActiveThemes(updateStylesSoon),
    lumine.styles.onDidUpdateStyleElement(updateStylesSoon),
    lumine.styles.onDidAddStyleElement(updateStylesSoon),
    lumine.styles.onDidRemoveStyleElement(updateStylesSoon),
    treeSitterHint(),
  );
}
...
function redraw(textEditor) {
  ...
  if (minimap) {
    lumine.views.getView(minimap).requestUpdate();
  }
}
...
function updateStyles() {
  ...
  const markersMoved = [...editorsMinimaps.values()].some((minimap) =>
    lumine.views.getView(minimap).markerStylesChanged(),
  );
  ...
  editorsMinimaps.forEach((minimap) => {
    ...
    const view = lumine.views.getView(minimap);
