...
class MinimapQuickSettingsElement extends HTMLElement {
  ...
  setModel(minimap) {
    ...
    this.subscriptions.add(
      ...
      Main.onDidRemovePlugin(({ name }) => {
        return this.removeItemFor(name);
      }),
      Main.onDidActivatePlugin(({ name }) => {
        return this.activateItem(name);
      }),
      Main.onDidDeactivatePlugin(({ name }) => {
        return this.deactivateItem(name);
      }),
