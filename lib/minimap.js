...
const EditorMetrics = require("./editor-metrics");
...
class Minimap {
  ...
  constructor(options = {}) {
     ...
     * the different APIs between different editor versions.
     *
     * @type {Object}
     * @access private
     */
    ...
    this.adapter = new EditorMetrics(this.textEditor);

    /**
     * When in independent scrolling mode, this value is used instead of the scroll computed from the editor.
     *
     * @type {number}
     * @access private
     */
    ...
    this.subscriptions.add(
      ...
      this.adapter.onDidChangeScrollTop(() => {
        if (!this.ignoreTextEditorScroll && !this.inChangeScrollTop) {
      ...
      // The package's own onDidDestroy subscription drops this editor from the
      // registry; this one only has to tear the model down.
   ...
   * have been changed in independent scrolling mode.
   *
  ...
  subscribeToConfig() {
    ...
    subs.add(
      lumine.config.observe("editor.scrollPastEnd", opts, (scrollPastEnd) => {
        this.scrollPastEnd = scrollPastEnd;
        this.adapter.scrollPastEnd = this.scrollPastEnd;
        this.emitter.emit("did-change-config");
      }),

      lumine.config.observe("minimap.charHeight", opts, (configCharHeight) => {
        this.configCharHeight = configCharHeight;
        this.updateScrollTop();
        this.emitter.emit("did-change-config");
      }),

      lumine.config.observe("minimap.charWidth", opts, (configCharWidth) => {
        this.configCharWidth = configCharWidth;
        this.updateScrollTop();
        this.emitter.emit("did-change-config");
      }),

      lumine.config.observe("minimap.interline", opts, (configInterline) => {
      ...
      lumine.config.observe(
        "minimap.independentMinimapScroll",
        opts,
      ...
      lumine.config.observe("minimap.scrollSensitivity", opts, (scrollSensitivity) => {
        this.scrollSensitivity = scrollSensitivity;
      }),

      lumine.config.observe("minimap.redrawDelay", opts, (redrawDelay) => {
        this.redrawDelay = redrawDelay;
      }),
      // cdprr is shorthand for configDevicePixelRatioRounding

      lumine.config.observe("minimap.devicePixelRatioRounding", opts, (cdprr) => {
  ...
  getTextEditorElement() {
    ...
    this.editorElement = lumine.views.getView(this.getTextEditor());
    return this.editorElement;
  }
  ...
  /**
   * Returns the height the minimap should take once displayed, which is the height of the `TextEditor`.
   *
  ...
  /**
   * Returns the width the Minimap should take once displayed, which is the width of its content.
   *
  ...
  /**
   * Sets the height and width the Minimap is displayed at.
   *
   ...
   */
  getScrollTop() {
    return this.independentMinimapScroll ? this.scrollTop : this.getScrollTopFromEditor();
  }

  /**
   * Sets the minimap scroll top value.
   *
   * @fires {did-change-scroll-top} if The Minimap scrolls independently of its editor
