...
import { Emitter, CompositeDisposable, Disposable, Range } from "atom";
...
export default class Minimap {
  ...
  constructor(options = {}) {
    ...
    this.subscriptions.add(
      ...
      this.adapter.onDidChangeScrollTop(() => {
        if (
          !this.standAlone &&
          !this.ignoreTextEditorScroll &&
          !this.inChangeScrollTop
        ) {
  ...
  getTextEditorScrollRatio() {
    return (
      this.adapter.getScrollTop() / (this.getTextEditorMaxScrollTop() || 1)
    );
  }
   ...
   */
  getHeight() {
    const additionalHeight = this.scrollPastEnd
      ? this.textEditor.getRowsPerPage()
      : 0;
    return (
      (this.textEditor.getScreenLineCount() + additionalHeight) *
      this.getLineHeight()
    );
  }
  ...
  onMouseWheel(event) {
    if (this.scrollIndependentlyOnMouseWheel()) {
      ...
      const updatedScrollTop =
        previousScrollTop - Math.round(wheelDeltaY * this.scrollSensitivity);
  ...
  onDidAddDecoration(...args) {
    return (
      this.getDecorationManagement()?.onDidAddDecoration(...args) ??
      disposedDisposable
    );
  }
  onDidRemoveDecoration(...args) {
    return (
      this.getDecorationManagement()?.onDidRemoveDecoration(...args) ??
      disposedDisposable
    );
  }
  onDidChangeDecorationRange(...args) {
    return (
      this.getDecorationManagement()?.onDidChangeDecorationRange(...args) ??
      disposedDisposable
    );
  }
  onDidUpdateDecoration(...args) {
    return (
      this.getDecorationManagement()?.onDidUpdateDecoration(...args) ??
      disposedDisposable
    );
  }
  decorationForId(...args) {
    return (
      this.getDecorationManagement()?.decorationForId(...args) ??
      dummyDecoration
    );
  }
  decorationsForScreenRowRange(...args) {
    return (
      this.getDecorationManagement()?.decorationsForScreenRowRange(...args) ??
      dummyDecoration
    );
  }
  decorationsByTypeThenRows() {
    return (
      this.getDecorationManagement()?.decorationsByTypeThenRows() ??
      dummyDecoration
    );
  }
  decorateMarker(...args) {
    return (
      this.getDecorationManagement()?.decorateMarker(...args) ?? dummyDecoration
    );
  }
  ...
  removeAllDecorationsForMarker(...args) {
    return this.getDecorationManagement()?.removeAllDecorationsForMarker(
      ...args,
    );
  }
