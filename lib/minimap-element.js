...
import { subscribeTo } from "./deps/event-helpers";
...
class MinimapElement extends HTMLElement {
  ...
  createdCallback() {
    ...
    this.subscriptions.add(
      ...
      atom.config.observe(
        "minimap.adjustMinimapWidthToSoftWrap",
        (adjustToSoftWrap) => {
      ...
      atom.config.observe(
        "minimap.useHardwareAcceleration",
  ...
  connectedCallback() {
    ...
    if (this.attachedToTextEditor) {
      this.minimap
        ...
        .setAttribute(
          "with-minimap",
          this.displayMinimapOnLeft ? "left" : "right",
        );
    }
    ...
    this.openQuickSettingSubscription = subscribeTo(this.openQuickSettings, {
      mousedown: (e) => {
        ...
        if (this.quickSettingsElement != null) {
          ...
          this.quickSettingsSubscription =
            this.quickSettingsElement.onDidDestroy(() => {
              this.quickSettingsElement = null;
            });

          const { top, left, right } =
            this.getFrontCanvas().getBoundingClientRect();
  ...
  update() {
...

    {
      ...
      const canvasTransform = makeScale(
        scale,
        scale,
        this.useHardwareAcceleration,
      );
  ...
  canvasLeftMousePressed(y) {
    ...
    const row =
      Math.floor(deltaY / this.minimap.getLineHeight()) +
    ...
    const scrollTop =
      row * textEditor.getLineHeightInPixels() -
  ...
  canvasMiddleMousePressed(y) {
    ...
    const ratio =
      deltaY /
      (this.minimap.getVisibleHeight() -
        this.minimap.getTextEditorScaledHeight());

    this.minimap.setTextEditorScrollTop(
      ratio * this.minimap.getTextEditorMaxScrollTop(),
    );
  }
  ...
  startDrag({ y, isLeftMouse, isMiddleMouse }) {
    ...
    const mousemoveHandler = (e) =>
      this.drag(extractMouseEventData(e), initial);
    const dragendHandler = () => this.endDrag();

    const touchmoveHandler = (e) =>
      this.drag(extractTouchEventData(e), initial);
