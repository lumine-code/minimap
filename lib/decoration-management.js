...
export default class DecorationManagement {
  ...
  decorateMarker(marker, decorationParams) {
    ...
    if (!this.decorationUpdatedSubscriptions.has(decoration.id)) {
      this.decorationUpdatedSubscriptions.set(
        decoration.id,
        decoration.onDidChangeProperties(() => {
  ...
  removeAllDecorations() {
    const decorationMarkerChangedSubscriptionsValues =
      this.decorationMarkerChangedSubscriptions.values();
    for (const decoration of decorationMarkerChangedSubscriptionsValues) {
      decoration.dispose();
    }

    const decorationMarkerDestroyedSubscriptionsValues =
      this.decorationMarkerDestroyedSubscriptions.values();
