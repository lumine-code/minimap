...
class CanvasDrawer extends HTMLElement {
  ...
  initializeCanvas() {
    ...
    /**
     * The marker layers other packages provide, drawn under the text.
     *
     * @type {MarkerCanvas}
     */
    this.markers = new MarkerCanvas({ className: "minimap-markers" });
  ...
  attachCanvases(parent) {
    // Before the tokens canvas, so the markers sit under the code rather than
    // over it: they are absolutely positioned siblings, and document order is
    // what stacks them.
    parent.appendChild(this.markers.element);
  ...
  drawMarkers(firstRow, lastRow, lineHeight) {
    const set = markerLayers.setFor(this.minimap.getTextEditor());
    const width = this.width;
    const height = this.height + lineHeight;
    if (!set || !(width > 0) || !(height > 0)) {
      return;
    }

    const regions = [];
    for (const layer of set.enabled()) {
    ...
    this.markers.draw(regions, width, height);
  }
