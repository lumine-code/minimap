...
export default class CanvasDrawer {
  ...
  initializeCanvas() {
    ...
    this.maxTokensInOneLine = atom.config.get(
      "minimap.maxTokensInOneLine",
    );
  }
  ...
  updateCanvas() {
    ...
    const decorations = this.DecorationManagement.decorationsByTypeThenRows(
      firstRow,
      lastRow,
    );
    ...
    backgroundDecorationDispatcher["background-custom"] =
...
function drawDecorations(
  ...
  renderData.context.clearRect(
    0,
    renderData.yRow,
    renderData.canvasWidth,
    renderData.lineHeight,
  );
  ...
  if (decorationsToRender != null ? decorationsToRender.length : undefined) {
    for (let i = 0, len = decorationsToRender.length; i < len; i++) {
      ...
      if (!SPEC_MODE) {
        ...
        const functionName = decorationDrawer.name;
...
function drawBackDecorationsForLines(
  ...
  for (let screenRow = firstRow; screenRow <= lastRow; screenRow++) {
    ...
    drawDecorations(
      ...
      decorations,
      renderData,
      backgroundDecorationDispatcher,
      editorElement,
    );
  }
...
function getDefaultColor(editorElement, textOpacity) {
  const color = styleReader.retrieveStyleFromDom(
    [".editor"],
    "color",
    editorElement,
    true,
...
function getDecorationColor(decoration, editorElement) {
  ...
  if (properties.scope) {
    ...
    return styleReader.retrieveStyleFromDom(
      scopeString,
      "background-color",
      editorElement,
      true,
